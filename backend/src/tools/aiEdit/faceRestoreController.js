...
async function restoreFace(req, res) {
  try {
    ...
    const { data: tool, error: toolError } = await supabase.from('tools_config').select('credit_cost').eq('tool_key', 'face_restore').eq('is_active', true).maybeSingle();
    if (toolError) throw toolError;
    if (!tool) return res.status(404).json({ success: false, message: 'Face restore tool is not configured.' });
    ...
    try {
      ...
      const pythonResponse = await fetch(`${PYTHON_AI_SERVICE_URL}/process/face-restore`, {
      ...
      if (fetchErr.name === 'AbortError') return res.status(504).json({ success: false, message: 'Face restore service timed out.' });
      return res.status(502).json({ success: false, message: 'Face restore service unavailable.' });
    }

    if (!pythonResult || !pythonResult.success) return res.status(500).json({ success: false, message: pythonResult?.message || 'Face restore failed.' });

    const outputBuffer = Buffer.from(pythonResult.imageBase64, 'base64');
    const adapter = await getActiveStorageAdapter();
    const storageConfig = await getStorageConfig();
    const fileName = `face_restore_${randomUUID()}.png`;
    ...
    const idempotencyKey = `${userId}_face_restore_${randomUUID()}`;
    const deduction = await deductCredits(userId, creditCost, 'face_restore', idempotencyKey);
    return res.json({ success: true, resultUrl, creditsDeducted: creditCost, newBalance: deduction.newBalance });
  } catch (error) {
    console.error('restoreFace:', error);
    return res.status(500).json({ success: false, message: error.message || 'Face restore failed.' });
  }
}

module.exports = { restoreFace };
