    ...
    <div className="mx-auto max-w-2xl px-6 py-10">
      ...
      <div className="mt-8 flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-[#26262E] py-16 text-center">
        <p className="text-sm text-[#9494A0]">
          This tool needs an AI provider connected — coming in a later phase.
        </p>
      </div>
    </div>
  );
...
function ToolRunner({ toolSlug, tool, config }) {
  ...
  const [params, setParams] = useState({ x: 0, y: 0, width: 300, height: 300, quality: 80 });
  ...
  async function handleRun() {
    ...
    try {
      ...
      const body = { imageUrl, ...numericParams() };
      const res = await api.post(`/tools/${config.apiPath}`, body);

      setResultUrl(res.data?.resultUrl);
      if (res.data?.newBalance !== undefined) {
        setUser((prev) => ({ ...prev, credits: res.data.newBalance }));
      }
      setStatus('done');
    } catch (err) {
      const message = err.response?.data?.message || 'Something went wrong.';
      if (err.response?.status === 402) {
        setErrorMsg(`Insufficient credits — you have ${err.response.data.available}, need ${err.response.data.required}.`);
      } else if (err.response?.status === 404) {
        setErrorMsg(`${message} (Admin needs to add this tool in the admin panel's Tools page first.)`);
      } else {
        setErrorMsg(message);
      }
      setStatus('idle');
  ...
  function numericParams() {
    const out = {};
    config.fields.forEach((f) => (out[f] = Number(params[f])));
    if (toolSlug === 'compress') out.format = 'jpeg';
    return out;
  }
      ...
      {!previewUrl ? (
        ...
        <div className="space-y-4">
              ...
              {config.fields.map((f) => (
                <label key={f} className="block">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-[#26262E] bg-[#15151C] px-3 py-2 text-sm outline-none focus:border-[#7C5CFC]"
                  />
                </label>
              ))}
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => { setFile(null); setPreviewUrl(null); setResultUrl(null); setErrorMsg(''); }}
              className="rounded-lg border border-[#26262E] px-4 py-2 text-sm hover:border-[#7C5CFC]/50"
            >
  ...
  <div className="space-y-2">
    ...
    <img
      src={resultUrl}
      alt="Result"
      className="max-h-80 w-full rounded-xl border border-[#26262E] object-contain"
    />

    <a
      href={resultUrl}
      download
      className="inline-flex items-center gap-1.5 rounded-lg border border-[#26262E] px-3 py-1.5 text-xs hover:border-[#2DD4BF]/50 hover:text-[#2DD4BF]"
    >
      <Download size={13} />
      Download
    </a>
